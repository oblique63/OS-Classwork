...
void print_values() {
    ...
    for (j=1; j <= 10; j++) {
        ...
        // provoke interleaved output
        sleep(1);
    }

    // With the 'kill' call commented out, the first process will no longer
    // call on the subsequent processes that are waiting for it, thus creating
    // a deadlock
